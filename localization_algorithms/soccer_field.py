...
class Field:
    ...
    def G(self, x, u):
        ...
        # jacobian = np.eye(3)
        #jacobian[:2, 2] = [-(trans * np.sin(prev_theta + rot1)), (trans * np.cos(prev_theta + rot1))]
        jacobian = np.array([[1, 0, -(trans * np.sin(prev_theta + rot1))], [0, 1, (trans * np.cos(prev_theta + rot1))], [0, 0, 1]])
        mu_pred = self.forward(x,u)
    ...
    def H(self, x, marker_id):
        ...
        d_x = (ly - prev_y) / ((lx - prev_x)**2 + (ly - prev_y)**2)
        d_y = (prev_x - lx) / ((lx - prev_x)**2 + (ly - prev_y)**2)
        jacobian = np.array([[d_x[0], d_y[0], -1]])
