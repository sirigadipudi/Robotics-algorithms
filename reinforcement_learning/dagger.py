...
def simulate_policy_dagger(env, policy, expert_paths, expert_policy=None, num_epochs=500, episode_length=50,
    ...
    for dagger_itr in range(num_dagger_iters):
        ...
        for i in range(len(trajs)):
          observations = observations + trajs[i]["observations"].tolist()
          actions = actions + trajs[i]["actions"].tolist()
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(num_batches):
                ...
                s_batch, a_batch = sample_batch(observations, actions, batch_size)
                s_batch = torch.tensor(s_batch).float().to(device)
                a_batch = torch.tensor(a_batch).float().to(device)
                a_hat = policy(s_batch)
                loss = torch.nn.functional.mse_loss(a_hat, a_batch)
                # TODO end
                loss.backward()
                optimizer.step()
