...
def train_model(policy, baseline, trajs, policy_optim, baseline_optim, gamma=0.99, baseline_train_batch_size=64, baseline_num_epochs=5):
    ...
    for traj in trajs:
        ...
        for t in reversed(range(len(rewards_singletraj)-1)):
            returns_singletraj[t] = rewards_singletraj[t] + gamma*returns_singletraj[t+1]
        # TODO end
    ...
    for epoch in range(baseline_num_epochs):
        ...
        for i in range(n // baseline_train_batch_size):
            ...
            # TODO start         
            states_batch = states[batch_index.cpu()]
            states_batch = torch.from_numpy(states_batch).float().to(device)
            returns_batch = returns[batch_index.cpu()]
            returns_batch = torch.from_numpy(returns_batch).float().to(device)
            baseline_predictions = baseline(states_batch)
            loss = criterion(baseline_predictions, returns_batch)
            # TODO end
    ...
    # TODO start
    advantage = torch.from_numpy(returns).to(device) - baseline_pred 
    loss = -(log_policy * advantage).mean()
    # TODO end
